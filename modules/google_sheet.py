    ...
    def get_sheet_files():
        ...
        for key, value in settings.items():
            if key != 'last_date_updated':
                print(f'{key}\n')
                if key == 'Clientes' or key == 'Financeiro':
                    sheet_files.append(value)
        return sheet_files
    ...
    def alter_values(sheet_files):
        ...
        def convert_value(value: str) -> float:
            ...
            if len(values) > 1:
                ...
                value2 = values[1]
                return int(float(value1) + float('0.' + value2))  # Converte e soma os valores como float
            else:
                value = value.replace('.', '')  # Remove o ponto
                return int(float(value))  # Converte o valor para float
